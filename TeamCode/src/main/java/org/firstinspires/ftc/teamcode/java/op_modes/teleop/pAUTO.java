...
@Autonomous(name="p AUTO TEST", group="Pushbot")
...
public class pAUTO extends LinearOpMode {
	...
	static final double     P_DRIVE_COEFF           = 0.000009;     // Larger is more responsive, but also less stable
	...
	public void runOpMode() {
		...
		while (!isStarted()) {
			telemetry.addData(">", "Robot Heading = %d",getHeading());
			telemetry.update();
		}
...
//		gyroDrive(DRIVE_SPEED,-48.0, 0.0);    // Drive REV 48 MMes
	 ...
	 * @param distance   Distance (in MMes) to move from current position.  Negative distance means move backwards.
